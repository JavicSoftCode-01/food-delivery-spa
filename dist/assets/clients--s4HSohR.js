import{O as y,g as w,n as E}from"./core-CP5HSxHI.js";import{h as T}from"./orders-CmWoKVYI.js";import"./vendor-D67DkiDD.js";import"fs";import"path";import"crypto";import"child_process";import"os";import"./ui-DeEF8Jzz.js";import"./utils-OsXyF5Oc.js";function F(l){var p;setTimeout(()=>window.scrollTo(0,0),0),l.innerHTML=`
    <div class="clients-container bg-white dark:bg-dark-bg-secondary rounded-xl p-3 border dark:border-dark-border space-y-3 mb-32 opacity-0 transform translate-y-4">
      ...
      <div id="ordersList" class="space-y-3"></div>
    </div>`;const u=l.querySelector(".clients-container");requestAnimationFrame(()=>{u.style.transition="all 0.5s cubic-bezier(0.4, 0, 0.2, 1)",u.style.opacity="1",u.style.transform="translateY(0)"});const m=l.querySelector("#inputFilterPhone"),s=l.querySelector("#ordersList"),b=(a,n=!1)=>{if(s.innerHTML="",!a.length){const e=document.createElement("div");e.className="text-gray-500 dark:text-gray-400 text-center py-8 opacity-0 transform translate-y-4",e.innerHTML=`
        ...
        </div>`,s.appendChild(t),n&&setTimeout(()=>{t.style.transition="all 0.5s cubic-bezier(0.4, 0, 0.2, 1)",t.style.opacity=e.delivered?"0.6":"1",t.style.transform="translateY(0)"},r*100)}),setTimeout(()=>{s.querySelectorAll(".deliveredToggle").forEach(e=>{e.addEventListener("change",r=>{const t=r.currentTarget,d=t.dataset.id,c=y.findById(d);c&&T(c,t,i=>{var x;const h=t.nextElementSibling,k=h.firstElementChild;h.className=`toggle-bg w-11 h-6 ${i?"bg-green-600":"bg-red-500"} rounded-full transition-all duration-300`,k.className=`toggle-dot absolute top-0.5 left-0.5 bg-white w-5 h-5 rounded-full transition-transform duration-300 ${i?"translate-x-5":"translate-x-0"}`;const f=(x=t.closest(".p-3"))==null?void 0:x.querySelector("span.inline-flex");f&&(f.className=`inline-flex items-center px-2 py-1 rounded-full text-sm font-medium transition-all duration-200 ${i?"bg-green-100 text-green-800 hover:bg-green-200":"bg-red-100 text-red-800 hover:bg-red-200"}`,f.innerHTML=`${i?'<i class="fa fa-check-circle mr-1 text-base"></i>Entregado':'<i class="fa fa-times-circle mr-1 text-base"></i>Pendiente'}`);const v=t.closest(".p-3");v&&(v.className=`order-item p-3 bg-gray-50 dark:bg-dark-bg rounded-lg border dark:border-dark-border hover:shadow-md transition-all duration-300 hover:scale-[1.02] ${i?"opacity-60":""}`)},()=>o(!1))})}),s.querySelectorAll(".callBtn").forEach(e=>{e.addEventListener("click",r=>{const t=r.currentTarget;t.style.transform="scale(0.95)",setTimeout(()=>{t.style.transform="",document.dispatchEvent(new CustomEvent("openCallModal",{detail:{phone:t.dataset.phone}}))},100)})}),s.querySelectorAll(".viewOrder").forEach(e=>{e.addEventListener("click",r=>{const t=r.currentTarget;t.style.transform="scale(0.95)",setTimeout(()=>{t.style.transform="",document.dispatchEvent(new CustomEvent("openOrderDetails",{detail:{id:t.dataset.id}}))},100)})}),s.querySelectorAll(".editOrder").forEach(e=>{e.addEventListener("click",r=>{const t=r.currentTarget;t.style.transform="scale(0.95)",setTimeout(()=>{t.style.transform="",document.dispatchEvent(new CustomEvent("openOrderForm",{detail:{id:t.dataset.id}}))},100)})})},a.length*100+100)},o=(a=!1)=>{const n=y.getSorted(),e=m.value.toLowerCase().trim();if(!e){b(n,a);return}const r=w(e),t=n.filter(d=>{const c=E(d.fullName).toLowerCase().includes(e);let g=!1;return r.length>0&&(g=w(d.phone).includes(r)),c||g});b(t,a)};m.addEventListener("input",()=>o(!1)),(p=l.querySelector("#btnAddOrder"))==null||p.addEventListener("click",a=>{const n=a.currentTarget;n.style.transform="scale(0.95)",setTimeout(()=>{n.style.transform="",document.dispatchEvent(new CustomEvent("openOrderForm"))},100)}),document.addEventListener("refreshViews",()=>o(!1)),o(!0)}export{F as renderClients};
