var Me=Object.defineProperty;var $e=Object.getOwnPropertySymbols;var Qe=Object.prototype.hasOwnProperty,Be=Object.prototype.propertyIsEnumerable;var Se=(r,e,t)=>e in r?Me(r,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):r[e]=t,K=(r,e)=>{for(var t in e||(e={}))Qe.call(e,t)&&Se(r,t,e[t]);if($e)for(var t of $e(e))Be.call(e,t)&&Se(r,t,e[t]);return r};var X=(r,e,t)=>new Promise((c,l)=>{var b=y=>{try{f(t.next(y))}catch(I){l(I)}},m=y=>{try{f(t.throw(y))}catch(I){l(I)}},f=y=>y.done?c(y.value):Promise.resolve(y.value).then(b,m);f((t=t.apply(r,e)).next())});import{U as s}from"./ui-DeEF8Jzz.js";import{O as w,u as _e,F as _,n as Ee,b as Ie,i as Y,t as Te,d as Re}from"./core-CiS44bps.js";import{a as Le,f as E}from"./utils-DWTgWmvX.js";function Ce(r,e,t,c){const l=e.checked,b=l?"ENTREGAR":"REVERTIR ENTREGA";e.checked=!l,t(!l);const m=`
  ...
  </div>`,I=()=>{const a=window.innerHeight*.01;document.documentElement.style.setProperty("--vh",`${a}px`)};I(),window.addEventListener("resize",I);const{close:U,element:u}=s.modal(y,{closeOnBackdropClick:!1}),M=()=>{U(),window.removeEventListener("resize",I)},o=u.querySelector('input[name="phone"]'),d=u.querySelector('select[name="foodId"]'),v=u.querySelector('select[name="comboId"]'),T=u.querySelector('input[name="quantity"]'),p=u.querySelector('input[name="comboQuantity"]'),N=u.querySelector("#totalPrice"),Q=u.querySelector("#unitPriceDisplay"),Z=u.querySelector("#cancelOrder"),q=u.querySelector("#orderForm"),D=q.querySelector('input[name="fullName"]'),B=q.querySelector('input[name="deliveryAddress"]'),O=q.querySelector('input[name="deliveryTime"]'),$=u.querySelector("#submitOrder");let W=null;const Pe=()=>{e&&(W={fullName:D.value,phone:o.value,deliveryAddress:B.value,deliveryTime:O.value,foodId:d.value,quantity:parseInt(T.value,10)||0,comboId:v.value,comboQuantity:parseInt(p.value,10)||0})},Ne=()=>({fullName:D.value,phone:o.value,deliveryAddress:B.value,deliveryTime:O.value,foodId:d.value,quantity:parseInt(T.value,10)||0,comboId:v.value,comboQuantity:parseInt(p.value,10)||0}),ee=()=>{if(!e||!W)return!0;const a=Ne();return JSON.stringify(W)!==JSON.stringify(a)},L=()=>{if(!e)return;if(b){$.disabled=!0,$.className="flex items-center gap-2 p-2 bg-gray-400 text-white rounded-lg cursor-not-allowed font-semibold text-base opacity-50",$.title="No se puede modificar un pedido entregado";return}const a=ee();$.disabled=!a,a?($.className="flex items-center gap-2 p-2 bg-accent text-white rounded-lg hover:bg-accent/90 font-semibold text-base transition-all duration-200",$.title="Guardar cambios"):($.className="flex items-center gap-2 p-2 bg-gray-400 text-white rounded-lg cursor-not-allowed font-semibold text-base opacity-50 transition-all duration-200",$.title="No hay cambios para guardar")},R=()=>{const a=d.selectedOptions[0],n=a?_.findById(a.value):null;if(!n){N.textContent=E(0),Q.textContent="",v.innerHTML='<option value="">Elegir combo</option>',p.disabled=!0;return}Q.textContent=`${E(n.price)}`;const i=v.value,h=parseInt(T.value,10)||0,k=parseInt(p.value,10)||0;let C=h*n.price;if(i){const x=n.combos.find(P=>P.id===i);x&&(C+=k*x.price),p.disabled=b}else p.disabled=!0;N.textContent=E(C)},te=a=>{const n=_.findById(a);v.innerHTML='<option value="">Sin combo</option>',n&&n.combos.length>0&&n.combos.forEach(i=>{const h=document.createElement("option");h.value=i.id,h.textContent=`${i.quantity} Uds. x ${E(i.price)}`,e&&e.comboId===i.id&&(h.selected=!0),v.appendChild(h)}),v.value&&(p.value=(e==null?void 0:e.comboQuantity.toString())||"1"),R(),L()},De=Re(()=>{const a=(()=>{const n=o.value.replace(/\s/g,"");return n.startsWith("09")||n.startsWith("+5939")||!n?null:"Formato válido: '09xxxxxxxx' o '+5939xxxxxxxx'"})();a&&s.toast(a)},3e3),Oe=(a,n)=>{const i=Math.abs(n)*60*1e3,h=w.getAll().filter(x=>!(e&&x.id===e.id)).map(x=>new Date(x.deliveryTime).getTime());let k=a,C=0;for(;;){const x=h.filter(j=>Math.abs(k-j)<i);if(x.length===0)break;const P=Math.max(...x);if(k=Math.max(k,P+i),C++,C>50)break}return k},ae=a=>X(null,null,function*(){var n;if(a.preventDefault(),e&&e.delivered){s.toast("❌ No se puede modificar un pedido que ya fue entregado");return}if(e&&!ee()){s.toast("ℹ️ No hay cambios para guardar");return}$.disabled=!0,s.showSpinner("Espere un momento...");try{const i=new FormData(q),h=Ee(String(i.get("fullName")||"")),k=Ie(String(i.get("phone")||"")),C=Ee(String(i.get("deliveryAddress")||"")),x=String(i.get("foodId")||""),P=parseInt(String(i.get("quantity")||"0"),10),j=String(i.get("comboId")||"")||null,xe=parseInt(String(i.get("comboQuantity")||"0"),10),ye=String(i.get("deliveryTime")||"");if(!ye){s.toast("Selecciona una hora de entrega.");return}let H="";try{const g=new Date,[S,F]=ye.split(":");g.setHours(parseInt(S,10),parseInt(F,10),0,0),H=g.toISOString()}catch(g){s.toast("Hora de entrega inválida.");return}if(!h||!k||!C){s.toast("Completa todos los campos requeridos.");return}if(!Y(k)){s.toast("El formato del teléfono es inválido."),o.focus(),o.classList.add("border-red-500");return}if(!x){s.toast("Selecciona una comida.");return}const V=_.findById(x);if(!V){s.toast("La comida seleccionada no fue encontrada.");return}let he=P;if(j){const g=V.combos.find(S=>S.id===j);g&&(he+=g.quantity*xe)}if(V.stock<he){s.toast(`Stock insuficiente para "${V.name}".`);return}if(P===0&&!j){s.toast("Agrega al menos un item o un combo al pedido.");return}const ke=Te(k);if(ke){const g=w.getAll().find(S=>e&&S.id===e.id?!1:Te(S.phone)===ke);if(g){s.toast(`El teléfono ya está registrado para el cliente '${g.fullName}'.`),o.focus(),o.classList.add("border-red-500");return}}const J=(n=JSON.parse(localStorage.getItem("fd_meta_v1")||"{}").deliveryGapMinutes)!=null?n:15;if(w.checkConflict(H,J,e==null?void 0:e.id).conflict){const g=new Date(H).getTime(),S=Oe(g,J),F=new Date(S),je=String(F.getHours()).padStart(2,"0"),Fe=String(F.getMinutes()).padStart(2,"0");O.value=`${je}:${Fe}`,s.toast(`Conflicto de entrega. Hora ajustada para cumplir ${J} min de intervalo.`,7e3),L();return}const we={fullName:h,phone:k,deliveryAddress:C,foodId:x,quantity:P,comboId:j,comboQuantity:xe,deliveryTime:H};let G=!1;if(e){G=yield w.update(K(K({},e),we)),G&&(s.toast("Pedido actualizado."),re(),M(),document.dispatchEvent(new CustomEvent("refreshViews")));return}else(yield w.add(we))&&(G=!0,s.toast("Pedido agregado."));if(G){document.dispatchEvent(new CustomEvent("refreshViews")),q.reset(),T.value="0",p.value="0",d.value="",v.innerHTML='<option value="">Sin combo</option>',o.value="",D.value="",B.value="";const g=new Date,S=String(g.getHours()).padStart(2,"0"),F=String(g.getMinutes()).padStart(2,"0");O.value=`${S}:${F}`,R(),L(),D.focus()}}finally{(!e||!b)&&($.disabled=!1),s.hideSpinner()}}),re=()=>{q.removeEventListener("submit",ae),Z.removeEventListener("click",oe),o.removeEventListener("input",le),o.removeEventListener("blur",ne),o.removeEventListener("focus",ie),d.removeEventListener("change",de),v.removeEventListener("change",ce),T.removeEventListener("input",ue),p.removeEventListener("input",fe),D.removeEventListener("input",A),B.removeEventListener("input",A),O.removeEventListener("change",A)},oe=()=>{re(),M()},Ae=a=>{const n=a.target;n.value=Ie(n.value)},se=()=>{const a=o.value;a.length===0?(o.classList.remove("border-green-500","border-red-500"),o.classList.add("border-gray-300")):Y(a)?(o.classList.remove("border-gray-300","border-red-500"),o.classList.add("border-green-500")):(o.classList.remove("border-green-500","border-red-500"),o.classList.add("border-gray-300"))},le=a=>{Ae(a),se(),De(),L()},ne=()=>{const a=o.value;a.length>0&&!Y(a)&&(o.classList.add("border-red-500"),o.classList.remove("border-gray-300","border-green-500"))},ie=()=>{o.classList.contains("border-red-500")&&(o.classList.remove("border-red-500"),o.classList.add("border-gray-300"))},de=()=>{te(d.value),z()},ce=()=>z(),ue=()=>z(),fe=()=>{parseInt(p.value,10)===0&&s.toast("La cantidad de combos debe ser mayor a 0."),z()},A=()=>L(),z=()=>{R(),L()};q.addEventListener("submit",ae),Z.addEventListener("click",oe),o.addEventListener("input",le),o.addEventListener("blur",ne),o.addEventListener("focus",ie),d.addEventListener("change",de),v.addEventListener("change",ce),T.addEventListener("input",ue),p.addEventListener("input",fe),D.addEventListener("input",A),B.addEventListener("input",A),O.addEventListener("change",A),e&&(te(e.foodId),setTimeout(()=>{Pe(),L()},100)),R(),se(),L()}const Ye=Object.freeze(Object.defineProperty({__proto__:null,openOrderForm:ze},Symbol.toStringTag,{value:"Module"}));function qe(r){const e=r.replace(/\D/g,"").startsWith("0")?`+593${r.replace(/\D/g,"").substring(1)}`:r.replace(/\s/g,""),t=`<div class="relative">
